...
export default function ChangeSettingsPage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get("http://1342/api/get-users-data");
  ...
  useEffect(() => {
    ...
    const fetchUserPermissions = async () => {
      try {
        const response = await axios.get(`http://1342/api/get-user-permissions/${selectedUser}`);
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://1342/api/change-user-settings", {
