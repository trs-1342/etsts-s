...
export default function ProductInfo() {
  ...
  useEffect(() => {
    const fetchKayit = async () => {
      try {
        const response = await fetch(
          `http://1342/api/getInfoProd/${fishNo}`,
          { credentials: "include" }
        );
  ...
  useEffect(() => {
    const fetchPrinters = async () => {
      try {
        const response = await fetch("http://1342/api/printers");
        if (!response.ok) {
...
tercih etmemeniz halinde 500 tl arıza tespit ücreti ödemeniz gerekecektir.`,
    ...
    try {
      const response = await fetch("http://1342/api/print", {
        method: "POST",
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (confirmDelete) {
                    try {
                      const response = await fetch(
                        `http://1342/api/deleteProduct/${kayit.fishNo}`,
                        {
                          method: "DELETE",
                          credentials: "include",
                        }
