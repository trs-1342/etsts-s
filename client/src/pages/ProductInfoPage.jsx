...
export default function ProductInfo() {
  ...
  //   (async () => {
  //     try {
  //       const response = await fetch("http://192.168.0.140:2431/api/printers");
    ...
    const checkAuthorization = async () => {
      try {
        const response = await fetch(
          `http://192.168.0.140:2431/api/check-product-access/${fishNo}`,
          {
            credentials: "include",
          }
        );
  ...
  useEffect(() => {
    const fetchkayit = async () => {
      try {
        const response = await fetch(
          `http://192.168.0.140:2431/api/getInfoProd/${fishNo}`,
          {
            credentials: "include",
          }
        );
  ...
  const handleRecord = async () => {
    ...
    try {
      const response = await fetch("http://192.168.0.140:2431/print", {
        method: "POST",
  ...
  //   try {
  //     const response = await fetch("http://192.168.0.140:2431/api/print", {
  //       method: "POST",
  //       headers: { "Content-Type": "application/json" },
  //       body: JSON.stringify({
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (confirmDelete) {
                try {
                  const response = await fetch(
                    `http://192.168.0.140:2431/api/deleteProduct/${kayit.fishNo}`,
                    {
                      method: "DELETE",
                      credentials: "include",
                    }
